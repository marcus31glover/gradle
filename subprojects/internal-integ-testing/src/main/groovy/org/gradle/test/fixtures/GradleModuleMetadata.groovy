        ...
        List<Dependency> getDependencies() {
            if (dependencies == null) {
                dependencies = (values.dependencies ?: []).collect {
                    ...
                    new Dependency(it.group, it.module, it.version?.requires, it.version?.prefers, it.version?.strictly, it.version?.rejects ?: [], it.version?.forSubgraph, exclusions, it.inheritConstraints, it.reason, it.artifactSelector, normalizeForTests(it.attributes))
                }
            }
            dependencies
        }
